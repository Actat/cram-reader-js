class CramSlice {
  ...
  async decodePositions(r) {
    ...
    if (
      this.container.compressionHeaderBlock
        .get("content")
        .get("pm")
        .get("AP") != 0
    ) {
      if (typeof this.last_position == "undefined") {
        this.last_position = this.sliceHeaderBlock
          .get("content")
          .get("alignmentStart");
      }
  ...
  async decodeNames(r) {
    if (
      this.container.compressionHeaderBlock.get("content").get("pm").get("RN")
    ) {
  ...
  async decodeTagData(r) {
    ...
    for (var elm of this.container.compressionHeaderBlock
      .get("content")
      .get("pm")
      .get("TD")[tagLine])
      await readTagFunc(elm);
    r.tags = tags;
  }
  ...
  async decodeMappedRead(r) {
    ...
    if (
      this.container.compressionHeaderBlock.get("content").get("dse").has("QS")
    ) {
  ...
  async decodeFeature(r) {
    ...
    f.set(
      ...
      String.fromCharCode.apply(
        "",
        new Uint8Array(await this.readItem("FC", "Byte"))
      )
    );
  ...
  async getBlockByExternalId(id) {
    ...
    var index = this.sliceHeaderBlock
      .get("content")
      .get("blockContentIds")
      .indexOf(id);
  ...
  async getBlocks() {
    if (typeof this.blocks == "undefined") {
      ...
      const numberOfBlocks = this.sliceHeaderBlock
        .get("content")
        .get("blockContentIds").length;
  ...
  async getRecords() {
    ...
    var records = [];
    const numberOfRecords = this.sliceHeaderBlock
      .get("content")
      .get("numberOfRecords");
  ...
  async getSliceHeaderBlock() {
    ...
    var b = await this.container.cram.readBlock(
      this.container.pos + this.container.headerLength + this.offset
    );
    ...
    b.get("content").set(
      "embeddedRefBasesBlockContentId",
      await data.readItf8()
    );
  ...
  async readItem(key, type = "Byte") {
    return await this.decodeItem(
      this.container.compressionHeaderBlock.get("content").get("dse").get(key),
      type
    );
  }
  ...
  async decodeItem(codec, type) {
    ...
    if (codecId == 1) {
      ...
      if (bl.length == 1 && bl[0] == 0) {
        ...
        console.error(
          "HUFFMAN decoding is in the process of being implemented.(cramSlice.decodeItem)"
        );
      }
    } else if (codecId == 5) {
      // BYTE_ARRAY_STOP
      ...
      // Int only
      return (
        this.blocks[0].get("IO").read(codec.get("length")) - codec.get("offset")
      );
    } else {
      console.error(
        "codecId: " +
          str(codecId) +
          " is not supported. (cramSlice.decodeItem())"
      );
      return;
    }
  }
