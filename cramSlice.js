class CramSlice {
    ...
    decodeTotalMateData(records) {
        for (var i = 0; i < records.length; i++) {
            ...
            if ((r.cf & 4) == 4) {
                ...
                var rmb =
                    r.readLength -
                    (r.hasSoftclip() ? r.features.get("S").length : 0); // mapped bases
                var nmb =
                    n.readLength -
                    (n.hasSoftclip() ? n.features.get("S").length : 0); // mapped bases
                var l = [
                    r.position,
    ...
    async decodeTagData(r) {
        ...
        const readTagFunc = async (elm) => {
            ...
            if (tagType == "A") {
                ...
                console.error(
                    "tagType'" + String(tagType) + "' is not supported."
                );
                return;
            }
    ...
    async decodeItem(codec, type) {
        ...
        if (codecId == 1) {
            ...
            io = (await this.getBlockByExternalId(codec.get("externalId"))).get(
                "IO"
            );
            if (type == "Int") {
                return io.readItf8();
            ...
            // Int only
            return (
                this.blocks[0].get("IO").read(codec.get("length")) -
                codec.get("offset")
            );
        } else {
            console.error(
